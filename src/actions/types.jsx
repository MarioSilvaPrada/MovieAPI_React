
// Movies Reducers
export const GET_GENRES = 'GET_GENRES';
export const GET_DISCOVER = 'GET_DISCOVER';
export const GET_CONFIG = 'GET_CONFIG';

// Fetch Reducers
export const GET_URL ='GET_URL';
export const FETCH_MOVIES = 'FETCH_MOVIES';
export const IS_LOADING = 'IS_LOADING'; 
export const IS_LOADING_MOVIE_INFO = 'IS_LOADING_MOVIE_INFO';

export const GENRE_SELECTED = 'GENRE_SELECTED';

// get Page
export const GET_PAGE = 'GET_PAGE';

export const SELECT_DISCOVER = 'SELECT_DISCOVER';

export const GET_MOVIE_INFO = 'GET_MOVIE_INFO';
export const SELECT_MOVIE_ID = 'SELECT_MOVIE_ID';

export const IS_LOADING_ACTOR_INFO = 'IS_LOADING_ACTOR_INFO';
export const SELECT_ACTOR_ID = 'SELECT_ACTOR_ID';
